...
- (void)setupSubViews
{
    ...
    self.layer.borderColor = UIColorGray8.CGColor;
...
- (void)layoutSubviews
{
    ...
    if (!self.isShowEmotionButton || !self.isShowMoreButton)
    {
        if (!self.isShowEmotionButton && !self.isShowMoreButton)
        {
            textView_w = self_w - 2 * kMargin_10;
            self.emotionButton.hidden = YES;
            self.moreButton.hidden = YES;
        }
        else if (!self.isShowEmotionButton )
        {
            textView_w = self_w - 3 * kMargin_10 - button_wh;
            self.emotionButton.hidden = YES;
    ...
    {
        textView_w = self_w - 4 * kMargin_10 - 2 * button_wh;
        self.emotionButton.hidden = NO;
        self.moreButton.hidden = NO;
    }
    
//    CGSize textViewSize = [self.commentTextView sizeThatFits:CGSizeMake(textView_w, CGFLOAT_MAX)];
//    self.commentTextView.frame = CGRectMake(kMargin_10, 5, textView_w, fminf(self.textViewMaximumHeight, fmaxf(textViewSize.height, self.textViewMinimumHeight)));
    
    self.commentTextView.frame = CGRectMake(kMargin_10, 7, textView_w, self.textViewMinimumHeight);
    self.emotionButton.frame = CGRectMake(CGRectGetMaxX(self.commentTextView.frame) + kMargin_10, self_h - 7 - button_wh , button_wh, button_wh);
    if (self.isShowEmotionButton && self.isShowMoreButton)
    {
        self.moreButton.frame = CGRectMake(CGRectGetMaxX(self.emotionButton.frame) + kMargin_10,  self_h - 7 - button_wh, button_wh, button_wh);
    }
    else
    {
        if (self.isShowMoreButton)
        {
            self.moreButton.frame = CGRectMake(CGRectGetMaxX(self.commentTextView.frame) + kMargin_10,  self_h - 7 - button_wh, button_wh, button_wh);
        }
    }
    self.emotionButton.centerY = self.commentTextView.centerY;
    self.moreButton.centerY = self.commentTextView.centerY;
    
    self.emotionView.frame = CGRectMake(0, SCREEN_HEIGHT - kEmotionViewHeight, SCREEN_WIDTH, kEmotionViewHeight);
    self.qqEmotionManager.emotionView.frame = self.emotionView.bounds;
    self.moreView.frame = CGRectMake(0, SCREEN_HEIGHT - kEmotionViewHeight, SCREEN_WIDTH, kEmotionViewHeight);
    ...
    switch (button.tag) {
            case BATimeLineKeyboardViewTypeEmotionButton:
        {
            ...
            if (button.selected)
            {
                ...
                [self updateKeyboardViewAnimationWithRect:CGRectMake(0, SCREEN_HEIGHT - self.height - kEmotionViewHeight, SCREEN_WIDTH, self.height)
...
- (QMUITextView *)commentTextView {
    if(_commentTextView == nil) {
        ...
        self.commentTextView.textColor = BAKit_ColorBlack;
        [self.commentTextView ba_setBordersWithColor:UIColorGray9 andCornerRadius:5.0f andWidth:0.5f];
...
- (UIView *)emotionView {
    if(_emotionView == nil) {
        ...
        self.emotionView.backgroundColor = BAKit_ColorWhite;
...
- (UIView *)moreView {
    if(_moreView == nil) {
        ...
        self.moreView.backgroundColor = BAKit_ColorWhite;
        _photoButton = [BAUIHelper ba_uiHelper_creatButtonWithTitle:@"照片" selTitle:nil titleColor:UIColorGray6 titleFontSize:14.0f image:BAKit_ImageName(@"photo_button") selImage:nil target:self selector:@selector(handleMoreViewButtonAction:)];
        _cameraButton = [BAUIHelper ba_uiHelper_creatButtonWithTitle:@"拍照" selTitle:nil titleColor:UIColorGray6 titleFontSize:14.0f image:BAKit_ImageName(@"camera_button") selImage:nil target:self selector:@selector(handleMoreViewButtonAction:)];
        _locationButton = [BAUIHelper ba_uiHelper_creatButtonWithTitle:@"位置" selTitle:nil titleColor:UIColorGray6 titleFontSize:14.0f image:BAKit_ImageName(@"location_button") selImage:nil target:self selector:@selector(handleMoreViewButtonAction:)];
        _videoButton = [BAUIHelper ba_uiHelper_creatButtonWithTitle:@"视频" selTitle:nil titleColor:UIColorGray6 titleFontSize:14.0f image:BAKit_ImageName(@"video_button") selImage:nil target:self selector:@selector(handleMoreViewButtonAction:)];
