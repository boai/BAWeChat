...
@implementation BAKit_JumpManager

+ (BAKit_JumpManager *)sharedBAKit_JumpManager
{
//    NSAssert(0, @"这是一个单例对象，请使用+(NetAPIManager *)sharedInstance方法");
    /*! 为单例对象创建的静态实例，置为nil，因为对象的唯一性，必须是static类型 */
    static id sharedBAKit_JumpManager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedBAKit_JumpManager = [[super allocWithZone:NULL] init];
    });
    return sharedBAKit_JumpManager;
}
...
{
    return [self sharedBAKit_JumpManager];
}
...
{
    if ([BAKit_Helper ba_helperIsNSDictionaryNULL:dict])
    {
        return;
    }
...
//    }];
    ...
    [BAKit_JumpManagerShare ba_loginAlertWithViewController:viewController];
}
...
- (void)ba_loginAlertWithViewController:(UIViewController *)viewController
{
    ...
    /*! 系统 alert */
    [UIAlertController ba_alertControllerShowAlertInViewController:viewController
                                       withTitle:@"温馨提示："
                          mutableAttributedTitle:title
                                         message:@"当前用户未登录，是否登录？"
                        mutableAttributedMessage:attributedMessage
                               buttonTitlesArray:@[@"取 消", @"登 录"]
                           buttonTitleColorArray:@[BAKit_ColorGreen, BAKit_ColorRed]
