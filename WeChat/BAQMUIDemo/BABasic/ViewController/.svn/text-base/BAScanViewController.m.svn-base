...
@implementation BAScanViewController
...
//    vc.strCodeType = strResult.strBarCodeType;
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2f * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        BAKit_StrongSelf
        if ([BAKit_RegularExpression ba_regularIsUrl:strResult.strScanned])
        {
            [self gotoWebWithUrl:strResult.strScanned];
        }
        else
        {
            NSString *msg = [NSString stringWithFormat:@"扫码结果：%@!", strResult.strScanned];
...
- (void)myQRCode
{
    BAQRViewController *vc = [BAQRViewController new];
